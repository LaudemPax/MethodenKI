\section{Beispielformulierungen von Zielen und Problemen}

\subsection{8er Puzzle (Sliding block puzzle)}

\begin{itemize}
    \item Hochgradig kombinatorisches NP-vollständiges Problem. Oft genutzt als Standardtest für neue Suchalgorithmen.
\end{itemize}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/8er-puzzle.png}
    \caption{8er puzzle Start- und Zielzustand}
    \label{fig:8er-puzzle}
\end{figure}

\begin{itemize}
    \item Zustände: Lokalität der 8 Fliesen in eine der 9 Flächen plus eine freie Kachel
    \item Operatoren: Blank nach Links, Rechts, Auf, Ab 
    \item Ziel-test: Blank-Kachel in der Mitte
    \item Pfadkosten: jeder Schritt kostet eine Einheit
\end{itemize}

\subsection{Staubsauger-Roboter}
\label{section:roomba}
Vieles an der Implementierung dieses Roboters muss abstrahiert werden:
\begin{itemize}
    \item World States: Umfassen alle Aspekte der reelen Welt
    \item Problem States: Nur Aspekte der relevant für das Problem sind. Die Modellierung von diesen Aspekten erfolgt meist in Form \textbf{symbolisher} Beschreibungen. 
    \item Zunächst müssen die möglichen World States als Problem States dargestellt werden:
\end{itemize}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/roomba-world-to-problem-states.png}
    \caption{Abbildung World States auf Problem States}
    \label{fig:roomba-abbildung}
\end{figure}

\subsubsection{Stark vereinfachte Staubsaugerwelt}

Eine sehr einfache Darstellung von einer Staubsaugerwelt hat zwei Orte. Jeder Ort kann entweder Staub enthalten oder nicht. Es gibt also 8 mögliche Zustände:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/staubsauger-vereinfacht.png}
    \caption{Staubsauger Zustände}
    \label{fig:roomba-simplified}
\end{figure}

Der Staubsauger kennt in diesem Fall 3 Operationen: Links, Rechts, Saugen und hat das Ziel, die Zustände S7 oder S8 zu erreichen, wo es keinen Staub mehr gibt.

\newpage
In diesem einfachen Fall können die Lösung mit Hilfe eines endlichen Automaten gefunden werden:

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/roomba-fsm.png}
    \caption{Staubsauger Pfadsucheautomat}
    \label{fig:roomba-fsm}
\end{figure}